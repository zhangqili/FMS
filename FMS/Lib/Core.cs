    ...
    public class Core
    {
                    ...
                    for (int y = 1; y <= sourceSheet.LastRowNum; y++)
                    {
                        if (Cell(sourceSheet, x, y) != null && Cell(sourceSheet, x + 1, y) != null)
                        {
                            ...
                            if (name != "")
                            {
                                ...
                                item.Score = Cell(sourceSheet, x + 1, y).NumericCellValue;
                                item.Rank = y;
                            }
                        }
        ...
        public void LegacyExport(string path)
        {
            ...
            for (int i = 0; i < DateItems.Count; i++)
            {
                ...
                for (int j = 0; j < item.Count; j++)
                {
                    ...
                    sheet.GetRow(j + 1).CreateCell(i * 3 + 1).SetCellValue(item.EffectiveListByDate[j].Score);
                }
                ...
                for (int i = startValue; i < list.Length; i++)
                {
                    ...
                    if (nameEnd > 0)
                    {
                        try
                        {
                            ...
                            score = double.Parse(line.Remove(0, nameEnd));
                            Item item = new Item() { Name = name, Score = score, DigitalDate = digitalDate };
                            items.Add(item);
                        }
                        catch (Exception)
                        {
                            Item item = new Item() { Name = "null", Score = 0, DigitalDate = digitalDate };
                            items.Add(item);
                        }
                    }
                }
        ...
        public void Save()
        {
            ...
            for (int i = 0; i < Items.Count; i++)
            {
                ...
                cells.CreateCell(1).SetCellValue(item.Score);
        ...
        private void AddItems(ISheet sheet)
        {
            ...
            for (int y = 1; y < sheet.LastRowNum; y++)
            {
                ...
                for (int x = 1; x < topRow.LastCellNum; x++)
                {
                    ...
                    item.Score = Cell(CoreSheet, x, y).NumericCellValue;
                }
            }
        }
        public void AddNameItem(string name)
        {
            ...
            {
                Item item = new Item() { Name = name, DigitalDate = dateItem.DigitalDate, Rank = 0, Score = 0, Change = "N/A" };
                Items.Add(item);
        ...
        List<Item> GetItems()
        {
            ...
            for (int i = 1; i <= CoreSheet.LastRowNum; i++)
            {
                ...
                items.Add(new Item()
                {
                    ...
                    Score = cells.Cells[1].NumericCellValue,
        ...
        private List<Item> GetSpecialList()
        {
            ...
            for (int i = 1; i <= SpecialSheet.LastRowNum; i++)
            {
                ...
                if (cells.GetCell(0).ToString() != "")
                    ...
                    {
                        Name = cells.GetCell(0).StringCellValue,
                        Score = Convert.ToInt32(cells.GetCell(1).NumericCellValue)
                    });
            }
            return items;
        }
        ...
        public List<DateItem> GroupByDate(List<Item> items)
        {
            ...
            foreach (var group in groups)
            {
                ...
                if (group.Key == 0)
                {
                    ...
                    {
                        MessageBox.Show(item.Name + item.Score.ToString());
                    }
                }
        ...
        public List<NameItem> GroupByName(List<Item> items)
        {
            ...
            foreach (var group in groups)
            {
                ...
                {
                    nameItem.CustomValue += SpecialList.Find(x => x.Name == group.Key).Score;
                }
        ...
        public void CalculateIntegalRank()
        {
            ...
            for (int i = 0; i < DateItems.Count; i++)
            {
                ...
                {
                    nameItems.First(x => x.Name == item.Name).CustomValue += item.Score;
                }
                nameItems = nameItems.OrderByDescending(x => x.CustomValue).ToList();
                for (int j = 0; j < nameItems.Count; j++)
                {
                    var item = new Item { Name = nameItems[j].Name, Rank = j + 1, DigitalDate = DateItems[i].DigitalDate, Score = nameItems[j].CustomValue };
